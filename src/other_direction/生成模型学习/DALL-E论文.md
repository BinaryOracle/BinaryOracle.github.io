...
此时，ELB 的优化变得困难：由于 $q_{\phi}$ 是离散分布，我们无法使用重参数化梯度来最大化它。通过结合在线聚类分配过程与直通估计器解决了这一问题。我们则使用 **Gumbel-Softmax 松弛**，将对 $q_{\phi}$ 的期望替换为对 $q_{\phi}^{\tau}$ 的期望，其中当温度 $\tau \to 0$ 时，该松弛会变得精确。$p_{\theta}$ 的似然使用 **对数拉普拉斯分布** 进行评估（推导见附录 A.3）。
